...
def load_registry(path: Path | None = None) -> dict[str, Any]:
    registry_path = (path or default_registry_path()).expanduser()
    ...
    if not isinstance(data, dict) or data.get("schema_version") != 1 or not isinstance(data.get("vaults"), dict):
...
def register_vault(
    ...
    *,
    ...
    path = (registry_path or default_registry_path()).expanduser()
    data = load_registry(path)
    existing = data["vaults"].get(vault_id)
    if existing is not None:
        if not isinstance(existing, dict) or not isinstance(existing.get("path"), str):
            raise ResolutionError(f"registry ID `{vault_id}` already exists with an invalid record")
        existing_root = Path(existing["path"]).expanduser().resolve()
        if existing_root != vault.root:
            raise ResolutionError(
                f"registry ID `{vault_id}` already points to `{existing_root}`; refusing to replace it with `{vault.root}`"
            )
