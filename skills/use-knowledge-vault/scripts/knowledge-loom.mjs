...
function resolveVaultPath(root, value) {
...
function resolveVaultPatternPrefix(root, value) {
  if (!isVaultRelativePath(value)) return null;
  const parts = [];
...
function splitFrontmatter(text, { source = "<text>" } = {}) {
...
function loadVault(root) {
  const rootPath = canonicalPath(root);
  const contractPath = path2.join(rootPath, CONTRACT_NAME);
  if (!fs2.statSync(contractPath, { throwIfNoEntry: false })?.isFile()) {
  ...
  const [contract, body] = splitFrontmatter(fs2.readFileSync(contractPath, "utf8"), { source: contractPath });
  return { root: rootPath, contractPath, contract, body };
}
function loadNoteFrontmatter(notePath) {
  try {
    return splitFrontmatter(fs2.readFileSync(notePath, "utf8"), { source: notePath })[0];
  } catch (error) {
    if (error instanceof ContractError) return {};
    throw error;
  }
}
function renderContract(data, body) {
  const yamlText = import_yaml.default.stringify(data, { lineWidth: 0 }).trimEnd();
  return `---
${yamlText}
---
...
function formatFindings(findings, { json = false } = {}) {
