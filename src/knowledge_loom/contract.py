...
def validate_contract_data(contract: dict[str, Any]) -> list[Finding]:
    ...
    if not isinstance(instruction_roots, list) or any(not isinstance(item, str) for item in instruction_roots):
        findings.append(Finding("error", "contract.instruction-roots", "`instruction_roots` must be a string list"))

    navigation = _mapping(contract, "navigation", findings)
    entrypoints = navigation.get("entrypoints")
    if not isinstance(entrypoints, list) or not entrypoints or any(not isinstance(item, str) for item in entrypoints):
        findings.append(Finding("error", "contract.navigation", "`navigation.entrypoints` must be a non-empty string list"))
    ...
    if not isinstance(profiles, dict):
        ...
        for name, profile in profiles.items():
            if not isinstance(profile, dict):
                ...
                continue
            if not isinstance(profile.get("paths"), list) or not profile["paths"]:
                findings.append(Finding("error", "contract.metadata-paths", f"profile `{name}` requires paths"))
    ...
    if not isinstance(focus_views, dict):
        ...
        for name, view in focus_views.items():
            if not isinstance(view, dict):
                ...
                continue
            if not isinstance(view.get("path"), str):
    ...
    if not isinstance(privacy, dict) or not isinstance(privacy.get("never_track", []), list):
